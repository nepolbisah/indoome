...
export const GET: APIRoute = async ({ site }) => {
  ...
  const defaultSitePublishedDate = import.meta.env.PUBLIC_SITE_PUBLISHED_DATE || currentTime; // Fallback untuk datePublished
  ...
  try {
    ...
    return new Response('Failed to load video data for video sitemap.', { status: 500 });
  }
  ...
  allVideos.forEach(video => {
    if (!video.id) {
        console.warn(`Skipping video without ID for sitemap: ${video.title || 'Unknown Title'}`);
        return;
    }
    ...
    // Use video.datePublished or fallback to defaultSitePublishedDate
    const videoPublicationDate = video.datePublished || defaultSitePublishedDate;
    ...
    if (video.title && video.description && absoluteThumbnailUrl && absoluteEmbedUrl) {
      ...
      videoEntries.push(`
        <url>
          ...
          <lastmod>${currentTime}</lastmod> <changefreq>weekly</changefreq>
          ...
          <video:video>
            ...
            <video:publication_date>${videoPublicationDate}</video:publication_date> ${tagsHtml}
